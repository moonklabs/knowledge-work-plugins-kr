...
ASM은 시맨틱이 풍부하지만 계층적입니다. 많은 시스템에서 평면 테이블이 필요합니다:
...
### 핵심 원칙
각 **측정**이 하나의 **행**이 됩니다. 메타데이터는 행마다 반복됩니다.

### 제외 항목
평탄화 시 다음과 같은 **최상위 ASM 메타데이터**는 의도적으로 제외됩니다:
- `$asm.manifest` (모델 버전, 스키마 URI)
- 기술 집합 문서 외부의 루트 수준 필드

이를 통해 출력이 실험 데이터에 집중됩니다. 규정 준수 또는 감사 목적으로 스키마 버전 추적이 필요한 경우, 원본 ASM JSON을 평탄화된 CSV와 함께 저장하거나 평탄화 스크립트를 수정하여 이 필드를 포함시키는 것을 고려하십시오.
...
- 열 이름을 LIMS 스키마 필드 이름에 맞추십시오
- 타임스탬프에 ISO 8601 날짜 형식을 사용하십시오
- 시료 ID가 기존 LIMS 시료 식별자와 일치하는지 확인하십시오
- LIMS가 단위를 별도 열에서 기대하는지 값에 포함하여 기대하는지 확인하십시오
