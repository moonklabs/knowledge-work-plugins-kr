...
> **참고:** 새 버전으로 업데이트할 때 [릴리스 페이지](https://github.com/nf-core/rnaseq/releases)에서 주요 변경 사항을 확인하고 아래 명령어의 버전을 업데이트하십시오.
...
| sample | 예 | 영숫자, 밑줄 허용 |
| fastq_1 | 예 | R1 절대 경로 |
| fastq_2 | 아니오 | R2 절대 경로 (단일 말단의 경우 비워둠) |
| strandedness | 예 | `auto`, `forward`, `reverse`, `unstranded` |

**가닥 특이성(strandedness) 가이드:**
- `auto`: 데이터에서 추론 (권장)
- `forward`: TruSeq Stranded, dUTP 프로토콜
- `reverse`: Ligation 기반 프로토콜
- `unstranded`: 비가닥 특이적 프로토콜
...
| `--pseudo_aligner` | - | 유사 정렬(pseudo-alignment)만 하려면 `salmon` 설정 |
| `--skip_trimming` | false | 어댑터 트리밍 건너뛰기 |
| `--skip_alignment` | false | 유사 정렬만 수행 |
...
**주요 출력:**
- `salmon.merged.gene_counts.tsv`: DESeq2/edgeR 입력용
- `salmon.merged.gene_tpm.tsv`: 정규화된 발현량
...
**STAR 인덱스 실패**: `--max_memory '64.GB'`로 메모리를 늘리거나 사전 빌드된 `--star_index`를 제공하십시오.

**낮은 정렬률**: 게놈이 종과 일치하는지 확인하고, FastQC에서 어댑터 오염을 확인하십시오.

**가닥 특이성 감지 실패**: `--strandedness reverse`로 명시적으로 지정하십시오.
