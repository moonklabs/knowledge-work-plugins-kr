...
description: >-
  ...
  for [company]". 이 모드에서는 스키마를 발견하고 핵심 질문을 한 뒤 참조 파일이 포함된 초기 스킬을 생성합니다. ITERATION MODE 트리거:
  "Add context about [domain]", "The skill needs more info about [topic]", "Update the data skill with
  [metrics/tables/terminology]", "Improve the [domain] reference". 이 모드에서는 기존 스킬을 불러오고 표적 질문을 한
  뒤 참조 파일을 추가하거나 갱신합니다. 데이터 분석가가 Claude에게 회사의 데이터 웨어하우스, 용어, 지표 정의,
  공통 쿼리 패턴을 이해시키려 할 때 사용합니다.
---
