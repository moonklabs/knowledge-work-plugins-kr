...
Data Analyst 플러그인입니다. 주로 Anthropic의 agentic 데스크톱 애플리케이션인 [Cowork](https://claude.com/product/cowork)용으로 설계되었지만 Claude Code에서도 동작합니다. SQL query, 데이터 탐색, 시각화, 대시보드, 인사이트 생성을 지원합니다. 어떤 data warehouse, SQL dialect, analytics stack에서도 사용할 수 있습니다.
...
이 플러그인은 Claude를 data analyst collaborator로 바꿉니다. Dataset exploration, 최적화된 SQL 작성, 시각화 생성, interactive dashboard 구축, 이해관계자 공유 전 분석 검증을 돕습니다.
...
- Copy/paste 없이 분석을 end-to-end로 실행합니다.
...
| 명령 | 설명 |
|---------|-------------|
| `/analyze` | 빠른 조회부터 전체 분석까지 data question에 답합니다. |
...
| `/build-dashboard` | 필터와 chart가 포함된 interactive HTML dashboard를 만듭니다. |
...
| `interactive-dashboard-builder` | Chart.js, 필터, styling을 사용해 HTML/JS dashboard를 구성합니다. |
...
You: /build-dashboard monthly revenue, top products,
     regional breakdown이 있는 sales dashboard를 만들어줘. 데이터는 여기 있어: [CSV 붙여넣기]
