...
사내 법무팀을 위한 AI 기반 생산성 플러그인입니다. 주로 Anthropic의 에이전트형 데스크톱 애플리케이션인 [Cowork](https://claude.com/product/cowork)용으로 설계되었지만 Claude Code에서도 동작합니다. 계약 검토, NDA 분류, 컴플라이언스 워크플로, 법무 브리핑, 템플릿 응답을 자동화하며, 조직의 업무 기준과 위험 허용도에 맞게 모두 설정할 수 있습니다.

> **면책 고지:** 이 플러그인은 법무 워크플로를 보조하지만 법률 자문을 제공하지 않습니다. 결론은 항상 자격을 갖춘 법률 전문가와 검증하세요. AI가 생성한 분석은 법적 의사결정에 사용하기 전에 면허 있는 변호사의 검토를 받아야 합니다. 이 플러그인의 기본 플레이북 예시는 미국 법적 입장과 관할(Delaware, New York, California)을 반영합니다. 다른 법체계(EU, UK, Netherlands, Australia 등)에서 운영한다면, 플러그인 분석에 의존하기 전에 `.claude/legal.local.md`의 플레이북을 해당 관할의 법적 요구사항, 표준 계약 조건, 컴플라이언스 의무에 맞게 커스터마이즈해야 합니다.

## 대상 사용자

- **상거래 법무 담당자** -- 계약 협상, 벤더 관리, 거래 지원을 담당하는 법무 담당자
- **제품 법무 담당자** -- 제품 검토, 이용약관, 개인정보 처리방침, IP 사안을 다루는 법무 담당자
- **개인정보 / 컴플라이언스 담당자** -- 데이터 보호 규제, DPA 검토, 정보주체 요청, 규제 모니터링 담당자
- **소송 지원 담당자** -- 증거보존 통지, 문서 검토 준비, 사건 브리핑 지원 담당자
...
### IP 소유권
- 표준 입장: 각 당사자는 기존 IP를 보유하고 고객 데이터는 고객이 소유
...
## 응답 템플릿
일반 문의용 템플릿 파일 경로를 설정하거나 인라인 템플릿을 정의하세요.
```
...
조직의 협상 기준으로 계약을 검토합니다. 표준과 다른 조항을 표시하고 수정안을 생성하며 사업 영향 분석을 제공합니다.

```
/review-contract
```

입력: 파일 업로드, URL, 붙여 넣은 계약 텍스트. 맥락(귀사 입장, 마감일, 집중 검토 영역)을 물은 뒤 설정된 플레이북 기준으로 조항별 검토를 수행합니다.
...
지원 문의 유형에는 정보주체 요청, 증거보존 요청, 벤더 질문, NDA 요청, 직접 정의한 사용자 지정 범주가 포함됩니다.

## 스킬

| 스킬 | 설명 |
|-------|-------------|
| `contract-review` | 플레이북 기반 계약 분석, 표준 이탈 분류, 수정안 생성을 수행합니다. |
| `nda-triage` | NDA 선별 기준, 분류 규칙, 라우팅 추천을 제공합니다. |
| `compliance` | 개인정보 규제(GDPR, CCPA), DPA 검토, 정보주체 요청을 다룹니다. |
| `canned-responses` | 템플릿 관리, 응답 범주, 에스컬레이션 트리거를 다룹니다. |
| `legal-risk-assessment` | 에스컬레이션 기준이 포함된 심각도-가능성 프레임워크로 법무 위험을 평가하고 분류합니다. 계약 위험 평가, 거래 노출 평가, 이슈 심각도 분류, 사안이 선임 법무 담당자 또는 외부 법무 검토를 필요로 하는지 판단할 때 사용합니다. |
| `meeting-briefing` | 법무 관련성이 있는 미팅을 위한 구조화된 브리핑을 준비하고 그에 따른 액션 아이템을 추적합니다. 계약 협상, 이사회 미팅, 컴플라이언스 검토처럼 법무 맥락, 배경 조사, 액션 추적이 필요한 미팅 준비에 사용합니다. |
...
3. 맥락을 제공합니다: "우리는 고객이고, 분기 말까지 체결해야 하며 데이터 보호와 책임 제한에 집중해야 합니다."
4. GREEN/YELLOW/RED 표시가 포함된 조항별 분석을 받습니다.
5. YELLOW 및 RED 항목에 대한 구체적인 수정 문구를 받습니다.
6. 분석 결과를 거래 팀과 공유합니다.

### NDA 분류

1. 영업팀이 신규 잠재 고객의 NDA를 보냅니다.
...
1. 사업팀이 기존 벤더와의 새 계약 건에 대해 묻습니다.
...
| 프로젝트 추적기 | Atlassian (Jira/Confluence) | 사안 추적, 작업 |

[CONNECTORS.md](CONNECTORS.md)에서 CLM, CRM, 전자서명 및 추가 선택지를 포함한 지원 통합 전체 목록을 확인하세요.

`.mcp.json`에서 연결을 설정합니다. 도구를 사용할 수 없으면 플러그인은 가능한 범위로 기능을 축소하고 공백을 표시하며 수동 확인을 제안합니다.

## 커스터마이징

### 플레이북 설정

플레이북은 계약 검토 시스템의 핵심입니다. `legal.local.md`에 입장을 정의하세요.

- **표준 입장**: 선호하는 계약 조건
- **허용 범위**: 에스컬레이션 없이 합의할 수 있는 범위
- **에스컬레이션 트리거**: 선임 검토 또는 외부 자문이 필요한 조건

### 응답 템플릿

일반 문의용 템플릿을 정의합니다. 템플릿은 변수 치환을 지원하며, 템플릿 응답을 사용하면 안 되는 상황을 위한 내장 에스컬레이션 트리거를 포함합니다.

### 위험 프레임워크

조직의 위험 선호도와 분류 체계에 맞게 위험 평가 매트릭스를 커스터마이즈합니다.
